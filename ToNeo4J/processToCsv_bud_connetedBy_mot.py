#!/usr/bin/env python3
"""Create relationship CSV between buildings and motor roads.

Rules:
- Load buildings from ntu_buildings.geojson and roads from ntu_roads.geojson.
- Limit roads to road_type in {service, primary, residential} (fallback to `highway` when needed).
- In projected CRS (SVY21, metres), detect if a building polygon intersects a road geometry or
  if their distance is < 10 metres.
- Record each matching pair as (building_osm_id, road_osm_id, relation="bud_conncetedBy_mot").

Output:
    neo4j_bud_connectedBy_mot.csv in the same folder as this script.
"""
...
OUTPUT_FILE = "/Users/admin/Desktop/6321PROJECT1/ToNeo4J/neo4j_bud_connectedBy_mot.csv"
MOTOR_TYPES = {"service", "primary", "residential"}
DISTANCE_THRESHOLD_M = 10.0
GEO_CRS = "EPSG:4326"
PROJ_CRS = "EPSG:3414"
RELATION_LABEL = "bud_conncetedBy_mot"
...
def load_geo_data():
    ...
    mot_roads = roads[roads["road_type"].str.lower().isin(t.lower() for t in MOTOR_TYPES)].copy().reset_index(drop=True)

    if mot_roads.empty:
        raise ValueError("在给定的 road_type 中没有找到机动车道路数据")

    return buildings, mot_roads
...
def main():
    try:
        ...
        print(f"生成 {len(rel_df)} 条建筑-机动车道路关系：{OUTPUT_FILE}")
        return 0
